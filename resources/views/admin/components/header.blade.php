<header class="bg-bgSecondary text-textHeading px-6 py-4 flex justify-between items-center shadow-md sticky top-0 z-5">
    ...
    <div class="flex items-center space-x-4">
        ...
        <!-- Notifications -->
        <div class="relative">
            <button class="text-textParagraph hover:text-textHeading" 
                onclick="showModal('notifications-modal')">
                <i class="fas fa-bell"></i>
            </button>
            @if(isset($notificationsCount) && $notificationsCount > 0)
                <span class="absolute -top-2 -right-2 h-5 w-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs">
                    {{ $notificationsCount }}
                </span>
            @endif
        </div>
        ...
        <div class="relative" x-data="{ open: false }">
            <div class="flex items-center space-x-3 cursor-pointer" @click="open = !open">
                <div class="avatar">
                    @if(auth()->user()->avatar)
                        <img src="{{ auth()->user()->avatar }}" alt="{{ auth()->user()->name }}" class="h-10 w-10 rounded-full object-cover">
                    @else
            ...
            <!-- Dropdown Menu -->
            <div class="absolute right-0 mt-2 w-48 bg-bgSecondary rounded-xl shadow-lg overflow-hidden z-10 border border-secondary hidden"
