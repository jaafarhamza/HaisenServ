...
<section class="relative py-16 overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10">
        ...
        <div class="text-center mb-16">
            <h2 class="text-3xl font-bold mb-4">All <span class="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Services</span></h2>
            <p class="text-gray-400 max-w-2xl mx-auto">Browse through our complete range of professional services, sorted by highest ratings.</p>
        </div>

        <!-- Filter Options -->
        <div class="mb-10">
            <div class="flex flex-wrap gap-4 justify-center mb-6">
                <button class="filter-btn active px-4 py-2 rounded-full bg-primary/20 border border-primary/30 text-white text-sm font-medium hover:bg-primary/30 transition-all" data-filter="all">All Services</button>
                @foreach($categories as $category)
                    <button class="filter-btn px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white text-sm font-medium hover:bg-white/10 transition-all" data-filter="category-{{ $category->id }}">{{ $category->name }}</button>
                @endforeach
            </div>
            <div class="flex flex-wrap gap-4 justify-center">
                <button class="sort-btn active px-4 py-2 rounded-full bg-primary/20 border border-primary/30 text-white text-sm font-medium hover:bg-primary/30 transition-all" data-sort="rating">Sort by Rating</button>
                <button class="sort-btn px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white text-sm font-medium hover:bg-white/10 transition-all" data-sort="price-low">Price: Low to High</button>
                <button class="sort-btn px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white text-sm font-medium hover:bg-white/10 transition-all" data-sort="price-high">Price: High to Low</button>
                <button class="sort-btn px-4 py-2 rounded-full bg-white/5 border border-white/10 text-white text-sm font-medium hover:bg-white/10 transition-all" data-sort="newest">Newest First</button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            @php
                ...
                if(isset($services)) {
                    ...
                    // Sort by average rating (highest first) by default
                    $services = $services->sortByDesc('average_rating');
                }
            @endphp
            ...
            @if(isset($services) && $services->count() > 0)
                @foreach($services as $index => $service)
                    ...
                    <div class="bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden border border-white/10 group hover:bg-white/10 transition-all duration-300 hover:-translate-y-2 hover:shadow-lg hover:shadow-{{ $colorTheme }}/10 service-card" 
                         ...
                         data-rating="{{ $service->average_rating }}" 
                         data-date="{{ $service->created_at->timestamp }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            filterButtons.forEach(btn => btn.classList.remove('active', 'bg-primary/20', 'border-primary/30'));
            filterButtons.forEach(btn => btn.classList.add('bg-white/5', 'border-white/10'));
            ...
            cards.forEach(card => {
                if (filter === 'all') {
                    ...
                    const categoryId = 'category-' + card.getAttribute('data-category');
    ...
    sortButtons.forEach(button => {
        button.addEventListener('click', function() {
            // Remove active class from all buttons
            sortButtons.forEach(btn => btn.classList.remove('active', 'bg-primary/20', 'border-primary/30'));
            sortButtons.forEach(btn => btn.classList.add('bg-white/5', 'border-white/10'));
...
.filter-btn.active::after, .sort-btn.active::after {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
